...
use think\Request;
use think\Session;
use think\auth\controller\Rbac;
use think\auth\model\ActionLog;
use think\auth\model\AuthAccess;
use think\auth\model\AuthRoleUser;
use think\auth\model\Menu;
...
class Auth
{
    ...
    public function auth()
    {
        ...
        $supperId = config('rbac.supper_id') ?: -1;
        if ($uid == $supperId) {
            self::actionLog($rule);
            return true;
        }
    ...
    public static function login($uid, $nickname)
    {
        ...
        $session_prefix = config('rbac.session_prefix');
        $user = [
            'uid'      => $uid,
            'nickname' => $nickname,
            'time'     => time()
